...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $invoice = getCurrentInvoice($_GET['number']);
...
<body class="container px-4 py-4">
  ...
  <div class="d-flex justify-content-between">
    <p>Edit a new invoice.</p>  
    <a class="text-decoration-none" href="index.php">< Back</a>
  </div>
  <div class="bg-body-tertiary p-5">
    <form class="form" method="post" enctype="multipart/form-data">
      <div class="mb-3">          
          <input type="hidden" class="form-control" name="number" value="<?php echo $invoice["number"]; ?>">
      </div>
      <div class="mb-3">
      ...
      <div class="mb-3">
          ...
          <input class="form-control" name="email" value="<?php echo $invoice['email'] ?>" placeholder="Email">
          <div class="error text-danger ps-2"><?php echo $errors['email'] ?? ''; ?></div>
      </div>
      <div class="mb-3">
          <label class="form-check-label" for="amount">Invoice Amount</label>
          <input class="form-control" name="amount" value="<?php echo $invoice['amount'] ?>" placeholder="Invoice Amount">
          ...
          <select class="form-select" name="status">
            ...
            <?php foreach($statuses as $status): ?>
              <?php if ($status != "all"): ?>
                ...
                <option value="<?php echo $status ?>" <?php if ($status == $invoice['status']) : ?> selected <?php endif; ?>><?php echo ucfirst($status) ?></option>
